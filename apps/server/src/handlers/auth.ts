...
import type { LoginPayload, CharacterSelectPayload, AuthSuccessPayload, CharacterInfoPayload } from '../protocol/types.ts';
import type { ClientSession } from '../ws/client-session.ts';
import { getCharactersByAccountId, getCharacterById } from '../game/character.ts';
import { getOrCreateMapInstance, registerOnlineCharacter, unregisterOnlineCharacter, cleanupEmptyMap, getMapInstance } from '../game/game-manager.ts';
import { getMap, getCompressedMap } from '../maps/map-store.ts';
import { getPathfinding } from '../maps/pathfinding.ts';

export async function handleLogin(session: ClientSession, payload: LoginPayload): Promise<void> {
  ...
  if (!account) {
    const msg = encodeServerMessage(ServerMessageType.AUTH_FAILURE, { reason: 'Invalid credentials' });
    session.ws.send(msg);
    return;
  }
...
export async function handleCharacterSelect(session: ClientSession, payload: CharacterSelectPayload): Promise<void> {
  if (!session.accountId) return;

  const character = await getCharacterById(payload.characterId);
  if (!character || character.account_id !== session.accountId) {
    session.ws.send(encodeServerMessage(ServerMessageType.ERROR, { reason: 'Character not found' }));
    return;
  }
  ...
  session.ws.send(encodeServerMessage(ServerMessageType.CHARACTER_INFO, charInfo));
  ...
  mapInstance.addActor(session, character.id, character.name, character.cell_id, character.direction, look);

  // Send all actors (including self) to the joining player
  const actors = mapInstance.getActors();
  session.ws.send(encodeServerMessage(ServerMessageType.MAP_ACTORS, { actors }));
